...
let activeSelection: LocationItem | null = null;
...
async function fetchCandidates(query: string): Promise<LocationItem[]> {
  ...
  const results: GeocodingResult[] = Array.isArray(data.results) ? data.results : [];
  return results
    .map((result) => ({
      name: result.name,
      admin: result.admin1,
...
function renderSuggestions(items: LocationItem[], query: string): void {
  if (!items.length) {
  ...
  suggestions.querySelectorAll("[data-suggestion-index]").forEach((el) => {
...
async function runSearch(query: string): Promise<void> {
  ...
  try {
    ...
    searchNote.textContent = items.length
      ? "Pick the exact city or region from the list."
      : "No matching locations found yet.";
  } catch (error) {
    if (token !== searchToken) return;
    suggestions.classList.remove("visible");
    suggestions.innerHTML = "";
    searchNote.textContent = (error as Error).message;
  }
}
